...
class Pick_Tshirt:
    #corner:gammas=[0.9]*4,pick_speed=0.005,move_speed=0.005,place_speed=0.005,lift_height=0.1
    def __init__(self):
        self.gamma=[1.0]
        self.pick_speed=0.005
        self.place_speed=0.005
        self.move_speed = 0.005
        self.lift_height=0.125
        self.primitive=["single"]
        self.lang_templates=[
            "Pick up the {which} of the T-shirt.",
            # "Seize the {which} of the T-shirt.",
            # "Grasp the {which} of the T-shirt.",
            # "Take hold of the T-shirt's {which}.",
            # "Capture the {which} of the T-shirt.",
            # "Hold onto the T-shirt's {which}.",
            # "Clutch the T-shirt's {which}.",
            # "Secure the {which} of the T-shirt.",
            # "Grip the T-shirt's {which}.",
            # "Snatch the {which} of the T-shirt.",
            # "Clasp the T-shirt's {which}.",
            # "Nab the T-shirt's {which}.",
            # "Clench the {which} of the T-shirt.",
            # "Pinch the T-shirt's {which}.",
            # "Snag the {which} of the T-shirt.",
            # "Tug on the T-shirt's {which}.",
        ]
        # self.position_templates={
        #     "left sleeve":["sleeve on the left side","the sleeve on the left","left-hand sleeve","sleeve on the left-hand side"],
        #     "right sleeve":["sleeve on the right side","the sleeve on the right","right-hand sleeve","sleeve on the right-hand side"],
        #     "left collar":["collar on the left side","the collar on the left","left-hand collar","collar on the left-hand side"],
        #     "right collar":["collar on the right side","the collar on the right","right-hand collar","collar on the right-hand side"],
        #     "left hem":["hem on the left side","the hem on the left","left-hand hem","hem on the left-hand side"],
        #     "right hem":["hem on the right side","the hem on the right","right-hand hem","hem on the right-hand side"],
        #     }
        self.act_templates={"left sleeve":2,
                            "right sleeve":5,
                            "left collar":0,
                            "right collar":1,
                            "left hem":6,
                            "right hem":7,}
    def get_action_instruction(self):
        pick_corners=["left sleeve","right sleeve","left collar","right collar","left hem","right hem"]
        pick_corner=random.choice(pick_corners)
        pick_idx=[self.act_templates[pick_corner]]
        instruction=[random.choice(self.lang_templates).format(which=pick_corner)]
        assert len(pick_idx)==len(self.gamma)==len(instruction)==len(self.primitive)
        return pick_idx,self.gamma,instruction,self.primitive
...
class Place_Tshirt:
    def __init__(self):
        ...
        self.primitive=["single"]
        self.lang_templates=[
            "Fold it to the {which}.",
            # "Position the fold to the {which}.",
            # "Bring it to the {which}.",
            # "Place it in the {which} by folding.",
            # "Arrange it to the {which}.",
            # "Fold it towards the {which}.",
            # "Guide it towards the {which} by folding.",
            # "Complete the fold,aligning it with the {which}.",
            # "Complete the fold,ensuring it reaches the {which}.",
            # "Direct the fold to the {which}.",
                    ]
        ...
        #     "left hem":["hem on the left side","the hem on the left","left-hand hem","hem on the left-hand side"],
        #     "right hem":["hem on the right side","the hem on the right","right-hand hem","hem on the right-hand side"],
        #     "the left middle part":["the central area on the left side","the left side midsection","the middle portion on the left","the left-central region"],
        #     "the right middle part":["the central area on the right side","the right side midsection","the middle portion on the right","the right-central region"],
        # }
        self.act_templates={"left sleeve":2,
                            "right sleeve":5,
                            "left collar":0,
                            ...
                            "left hem":6,
                            "right hem":7,
                            "the left middle part":3,
                            "the right middle part":4,}
    def get_action_instruction(self):
        place_corners=["left sleeve","right sleeve","left collar","right collar","left hem","right hem","the left middle part","the right middle part"]
        place_corner=random.choice(place_corners)
        place_idx=[self.act_templates[place_corner]]
        #place_position=random.choice(self.position_templates[place_corner])
        instruction=[random.choice(self.lang_templates).format(which=place_corner)]
        assert len(place_idx)==len(self.gamma)==len([instruction])==len(self.primitive)
        return place_idx,self.gamma,instruction,self.primitive
...
Demonstrator = {
    ...
    "Pick_Tshirt": Pick_Tshirt,
    "Place_Tshirt":Place_Tshirt
}
