...
def train_net(configs):
    ...
    for epoch in tqdm(range(already_trained_epochs, already_trained_epochs + epochs)):
        ...
        # train model
            ...
            total_pick_loss = 0
            total_place_loss = 0
            for batch, (depths, graph_datas, position_heatmaps, instructions, _) in enumerate(pbar):
                depths, graph_datas, position_heatmaps, instructions = (
                    depths.to(device),
                    graph_datas.to(device),
                    # pick_heatmaps.to(device),
                    # place_heatmaps.to(device),
                    position_heatmaps.to(device),
                    instructions.to(device),
                )
                # forward & loss
                if model_type == "depthonly":
                    #pred_pick_heatmaps, pred_place_heatmaps, _ = model(instructions, depths)
                    pred_position_heatmaps,_=model(instructions,depths)
                elif model_type == "graphdepth":
                    #pred_pick_heatmaps, pred_place_heatmaps, _ = model(instructions, depths, graph_datas)
                    pred_position_heatmaps,_=model(instructions,depths,graph_datas)
                ...
                loss=loss_fn(pred_position_heatmaps,position_heatmaps)
