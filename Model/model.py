...
class DepthOnly(nn.Module):
    def __init__(
        ...
        self.position_decoder = ConvDecoder(dim)

        # frozen clip & graph_encoder
        self.frozen_clip()

    def load_model(self, model_path):
        map_location = {"cuda:2": "cuda:0"}  # 模拟GPU设备名称发生了变化，需要通过字典映射设备名称
        self.load_state_dict(torch.load(model_path,map_location=torch.device("cuda:0"))["model"])
    ...
    def forward(self, text, img):
        ...
        position_heatmap = self.position_decoder(image_features).squeeze()

        return position_heatmap, all_head
...
class GraphOnly(nn.Module):
    ...
    def load_model(self, model_path):
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  
        self.load_state_dict(torch.load(model_path)["model"],map_location=device)
        # self.load_state_dict(torch.load(model_path)["model"])
        print(f"load agent from {model_path}")
