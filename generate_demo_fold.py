...
def generate_demos(cached_path, save_root, task, gui, cloth3d, randomize_pose, num_demonstrations):
    ...
    for i in tqdm(range(num_demonstrations)):
        ...
        elif task=="Pick_Tshirt":
            pick_idx,gamma,action_instruction,action_primitive=policy.get_action_instruction()
            instructions+=action_instruction
            primitives+=action_primitive
        elif task=="Place_Tshirt":
            place_idx,gamma,action_instruction,action_primitive=policy.get_action_instruction()
            instructions+=action_instruction
            primitives+=action_primitive
        else:
        ...
        if task=="Pick_Tshirt":
            ...
            place_idx=[random.choice(range(8))]
            while pick_idx[0]==place_idx[0]:
                place_idx=[random.choice(range(8))]
            pick_pos=keypoints_pos[pick_idx[0]]
            place_pos=keypoints_pos[place_idx[0]]
            #place_pos=keypoints_pos[random.choice(range(8))]
            ...
            print(print(pick_pixels[0],primitives[0],instructions[0]))
            ...
            keypoints_pos=env.get_keypoints(keypoints_index)
            pick_idx=[random.choice([0,1,2,5,6,7])]
            while pick_idx[0]==place_idx[0]:
                pick_idx=[random.choice([0,1,2,5,6,7])]
            pick_pos=keypoints_pos[pick_idx[0]]
            place_pos=keypoints_pos[place_idx[0]]
            ...
            # with open(os.path.join(task_root,str(max_index+i),"test.txt"),"w") as t:
            #     t.write(place_pixels[0])
            #     t.write(primitives[0])
            #     t.write(instructions[0])
            print(place_pixels[0],primitives[0],instructions[0])

            with open(os.path.join(task_root, str(max_index + i), "info.pkl"), "wb+") as f:
                data = {"place": place_pixels, 
                        "primitive": primitives, 
                        "instruction": instructions,}
                pickle.dump(data, f)
        else:
        ...
        with open(os.path.join(task_root,str(max_index+i),"viz","instruction"+".txt"),"w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_demonstrations", type=int, default=10000, help="number of demonstrations")
